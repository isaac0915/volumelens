...
- `app/main.py` — FastAPI entry point; `lifespan` starts the stock poller and radar scanner; all HTTP routes (`/api/stocks`, `/api/quotes`, `/api/market`, `/api/daily-spikes`, `/api/search`, `/api/alerts`, `/api/stocks/{symbol}/detail`). `/api/quotes?symbols=` serves arbitrary watchlists: latest daily close per symbol, overlaid with live MIS quotes during market hours (per-symbol 10s cache, misses cached too; unknown symbols dropped; max 50). `/api/market` returns TAIEX/TPEx indices from MIS (`tse_t00`/`otc_o00`, 15s stale-while-revalidate cache refreshed by a background task) plus `tracked_symbols` and `latest_session`. `/api/daily-spikes` finds latest-trading-day spikes from `stock_candles` (volume ≥2× the previous 20 days via `AVG() OVER (ROWS BETWEEN 20 PRECEDING AND 1 PRECEDING)`, ≥500 lots), so it works outside market hours unlike the intraday radar
...
- `frontend/app/icon.svg`, `apple-icon.js`, `opengraph-image.js` — favicon and generated link-preview images; `layout.js` sets `metadataBase` from `NEXT_PUBLIC_SITE_URL` (set it when deploying so OG image URLs are absolute)
- `frontend/lib/watchlist.js` + `components/WatchButton.js` — per-browser watchlist (localStorage via `useSyncExternalStore`, default `2330`/`2317`, max 50) and the star toggle used on ranking rows, scanner tables and the stock page. Keep the star *beside* row `<Link>`s, not inside them (no buttons nested in links)
- `frontend/app/page.js` — Overview dashboard: product intro hero with coverage stats, index cards (`/api/market`), the viewer's watchlist (`/api/quotes`, 5s), and 收盤爆量排行 (`/api/daily-spikes`); shows 盤中 vs 已收盤 from `market_open`
